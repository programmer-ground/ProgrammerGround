...
public class OAuthMemberService {
	 ...
	 * star
	 */
	public GithubTotalDto getGithubTotal() throws ExecutionException, InterruptedException {
		//Spring Seucrity UserDetails 객체 가져오기
		MyUserDetails userDetails = (MyUserDetails)((JwtAuthenticationToken)SecurityContextHolder.getContext()
			.getAuthentication()).getPrincipal();
		Oauth2AuthorizedClient oauthUser = oAuthMemberRepository.findById(userDetails.getId())
			.orElseThrow()
		...
		CompletableFuture<List<GithubRepoDto>> repoFuture = CompletableFuture.supplyAsync(
